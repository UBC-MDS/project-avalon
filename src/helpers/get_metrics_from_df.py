...
def get_metrics_from_df(df, target_column: str, required_metrics: list = ["mae", "mse"]):
    """
    ...
    target_column (str): The name of the column in df that contains the actual values.
    ...
    for col in df.columns[1:]:
        if require_mae:
            mae = mean_absolute_error(df[target_column], df[col])
            mae_values.append(mae)
        if require_mse:
            mse = mean_squared_error(df[target_column], df[col])
            mse_values.append(mse)
        if require_mape:
            mape = mean_absolute_percentage_error(df[target_column], df[col])
